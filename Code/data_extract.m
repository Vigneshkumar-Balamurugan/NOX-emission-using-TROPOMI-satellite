...
if numel(fieldnames(tropomi_struct)) >=1
    ...
    if length(fie_names) >= 2
        disp('There is more than one TROPOMI file available for this investigation date. Below are the beginning parts of the filenames: ')
        disp(fie_names)
        user_option = input('Choose which file to investigate (1, 2, 3, etc.): ');
    else
        user_option = 1;
    end
